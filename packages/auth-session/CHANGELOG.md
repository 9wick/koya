# Changelog

## [0.6.0](https://github.com/zeltjs/zelt/compare/auth-session-v0.5.0...auth-session-v0.6.0) (2026-05-17)


### Miscellaneous Chores

* **auth-session:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.6.0
    * @zeltjs/kv bumped to 0.6.0
  * peerDependencies
    * @zeltjs/core bumped to 0.6.0
    * @zeltjs/kv bumped to 0.6.0

## [0.5.0](https://github.com/zeltjs/zelt/compare/auth-session-v0.4.0...auth-session-v0.5.0) (2026-05-17)


### Bug Fixes

* **build:** transform TC39 decorators with SWC at build time ([c681663](https://github.com/zeltjs/zelt/commit/c6816636d04e85690115fd64c00aaf02a11fc70b))
* **build:** transform TC39 decorators with SWC at build time ([3430ff3](https://github.com/zeltjs/zelt/commit/3430ff33f0dc31981a7da55246c7ca43baa2d281))


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.5.0
    * @zeltjs/kv bumped to 0.5.0
  * peerDependencies
    * @zeltjs/core bumped to 0.5.0
    * @zeltjs/kv bumped to 0.5.0

## [0.4.0](https://github.com/zeltjs/zelt/compare/auth-session-v0.3.0...auth-session-v0.4.0) (2026-05-17)


### Miscellaneous Chores

* **auth-session:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.4.0
    * @zeltjs/kv bumped to 0.4.0
  * peerDependencies
    * @zeltjs/core bumped to 0.4.0
    * @zeltjs/kv bumped to 0.4.0

## [0.3.0](https://github.com/zeltjs/zelt/compare/auth-session-v0.2.1...auth-session-v0.3.0) (2026-05-17)


### ⚠ BREAKING CHANGES

* **kv:** KV methods now return Promise instead of ResultAsync. namespace() returns KVStore directly instead of Result<KVStore>.

### Features

* add @zeltjs/redis and @zeltjs/eventbus packages ([a84741c](https://github.com/zeltjs/zelt/commit/a84741c194935601239693730694370c3a33d714))
* **auth:** add JWT driver option and session package ([1480da1](https://github.com/zeltjs/zelt/commit/1480da1cac6b0bb067b3a4d9ef8e97d9c6813620))
* **auth:** add JWT driver option and session package ([6d85c56](https://github.com/zeltjs/zelt/commit/6d85c566e790f2797fdd6e84e72c73388c1457de))
* **core:** add structured error classes with Zelt prefix ([24e0a75](https://github.com/zeltjs/zelt/commit/24e0a7523e82d353dcb38f8a37d421bd44faccb9))
* **core:** add structured error classes with Zelt prefix ([955da70](https://github.com/zeltjs/zelt/commit/955da702cb843e7ae977982cf6c541a8dcddd11c))
* **core:** add TC39/legacy decorator dual-mode support ([45e779e](https://github.com/zeltjs/zelt/commit/45e779ebba611fc7ae30a499f332a4268125dbcb))
* **kv:** remove neverthrow from public API ([eba2042](https://github.com/zeltjs/zelt/commit/eba2042735c3ca73545813c76d5f4a2dc0f82bf6))


### Bug Fixes

* add kv reference to auth-session tsconfig ([4278533](https://github.com/zeltjs/zelt/commit/4278533f690cca9ab99b0f5ef117d3bb673980cc))
* remove unused dependencies from auth-session package ([10e53e0](https://github.com/zeltjs/zelt/commit/10e53e0d43ff1ee4fab98db58eb3295fdc0950b9))
* **scripts:** correct npm trust command flag and improve interactive flow ([1d979e3](https://github.com/zeltjs/zelt/commit/1d979e3df81b77230dfa55819876b64019c49683))
* **scripts:** correct npm trust setup and add interactive flow ([ec493a6](https://github.com/zeltjs/zelt/commit/ec493a642972c55e93eb7d3ec34fa3024047484a))


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.3.0
    * @zeltjs/kv bumped to 0.3.0
  * peerDependencies
    * @zeltjs/core bumped to 0.3.0
    * @zeltjs/kv bumped to 0.3.0

## [0.2.1](https://github.com/zeltjs/zelt/compare/auth-session-v0.1.0...auth-session-v0.2.1) (2026-05-17)


### ⚠ BREAKING CHANGES

* **kv:** KV methods now return Promise instead of ResultAsync. namespace() returns KVStore directly instead of Result<KVStore>.

### Features

* add @zeltjs/redis and @zeltjs/eventbus packages ([a84741c](https://github.com/zeltjs/zelt/commit/a84741c194935601239693730694370c3a33d714))
* **auth:** add JWT driver option and session package ([1480da1](https://github.com/zeltjs/zelt/commit/1480da1cac6b0bb067b3a4d9ef8e97d9c6813620))
* **auth:** add JWT driver option and session package ([6d85c56](https://github.com/zeltjs/zelt/commit/6d85c566e790f2797fdd6e84e72c73388c1457de))
* **core:** add structured error classes with Zelt prefix ([24e0a75](https://github.com/zeltjs/zelt/commit/24e0a7523e82d353dcb38f8a37d421bd44faccb9))
* **core:** add structured error classes with Zelt prefix ([955da70](https://github.com/zeltjs/zelt/commit/955da702cb843e7ae977982cf6c541a8dcddd11c))
* **core:** add TC39/legacy decorator dual-mode support ([45e779e](https://github.com/zeltjs/zelt/commit/45e779ebba611fc7ae30a499f332a4268125dbcb))
* **kv:** remove neverthrow from public API ([eba2042](https://github.com/zeltjs/zelt/commit/eba2042735c3ca73545813c76d5f4a2dc0f82bf6))


### Bug Fixes

* add kv reference to auth-session tsconfig ([4278533](https://github.com/zeltjs/zelt/commit/4278533f690cca9ab99b0f5ef117d3bb673980cc))
* remove unused dependencies from auth-session package ([10e53e0](https://github.com/zeltjs/zelt/commit/10e53e0d43ff1ee4fab98db58eb3295fdc0950b9))
* **scripts:** correct npm trust command flag and improve interactive flow ([1d979e3](https://github.com/zeltjs/zelt/commit/1d979e3df81b77230dfa55819876b64019c49683))
* **scripts:** correct npm trust setup and add interactive flow ([ec493a6](https://github.com/zeltjs/zelt/commit/ec493a642972c55e93eb7d3ec34fa3024047484a))


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.2.1
    * @zeltjs/kv bumped to 0.2.1
  * peerDependencies
    * @zeltjs/core bumped to 0.2.1
    * @zeltjs/kv bumped to 0.2.1
