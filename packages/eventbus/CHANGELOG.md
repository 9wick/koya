# Changelog

## [0.6.0](https://github.com/zeltjs/zelt/compare/eventbus-v0.5.0...eventbus-v0.6.0) (2026-05-17)


### Miscellaneous Chores

* **eventbus:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.6.0
    * @zeltjs/redis bumped to 0.6.0
  * peerDependencies
    * @zeltjs/core bumped to 0.6.0
    * @zeltjs/redis bumped to 0.6.0

## [0.5.0](https://github.com/zeltjs/zelt/compare/eventbus-v0.4.0...eventbus-v0.5.0) (2026-05-17)


### Miscellaneous Chores

* **eventbus:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.5.0
    * @zeltjs/redis bumped to 0.5.0
  * peerDependencies
    * @zeltjs/core bumped to 0.5.0
    * @zeltjs/redis bumped to 0.5.0

## [0.4.0](https://github.com/zeltjs/zelt/compare/eventbus-v0.3.0...eventbus-v0.4.0) (2026-05-17)


### Miscellaneous Chores

* **eventbus:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.4.0
    * @zeltjs/redis bumped to 0.4.0
  * peerDependencies
    * @zeltjs/core bumped to 0.4.0
    * @zeltjs/redis bumped to 0.4.0

## [0.3.0](https://github.com/zeltjs/zelt/compare/eventbus-v0.2.1...eventbus-v0.3.0) (2026-05-17)


### Miscellaneous Chores

* **eventbus:** Synchronize zeltjs versions


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.3.0
    * @zeltjs/redis bumped to 0.3.0
  * peerDependencies
    * @zeltjs/core bumped to 0.3.0
    * @zeltjs/redis bumped to 0.3.0

## [0.2.1](https://github.com/zeltjs/zelt/compare/eventbus-v0.1.0...eventbus-v0.2.1) (2026-05-17)


### Features

* add @zeltjs/redis and @zeltjs/eventbus packages ([a84741c](https://github.com/zeltjs/zelt/commit/a84741c194935601239693730694370c3a33d714))
* add @zeltjs/redis and @zeltjs/eventbus packages ([0979e0e](https://github.com/zeltjs/zelt/commit/0979e0e86601936861782076e98ba5cd74eb0b8c))


### Bug Fixes

* rename redis-event-bus-adaptor to redis-event-bus.adaptor ([d81e7e0](https://github.com/zeltjs/zelt/commit/d81e7e07e677d68841bee9c5fe4e9fa9c49e1d73))
* **scripts:** correct npm trust command flag and improve interactive flow ([1d979e3](https://github.com/zeltjs/zelt/commit/1d979e3df81b77230dfa55819876b64019c49683))
* **scripts:** correct npm trust setup and add interactive flow ([ec493a6](https://github.com/zeltjs/zelt/commit/ec493a642972c55e93eb7d3ec34fa3024047484a))


### Dependencies

* The following workspace dependencies were updated
  * devDependencies
    * @zeltjs/core bumped to 0.2.1
    * @zeltjs/redis bumped to 0.2.1
  * peerDependencies
    * @zeltjs/core bumped to 0.2.1
    * @zeltjs/redis bumped to 0.2.1
